...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha.defaults = {
		...
		onSuccess: function($form, $canvas, ctx) { //성공 했을 경우 변경 가능
			var opts = this,//김은빈  $form.find('button[id=\"Login_btn\"]:disabled'); ->$('button[id=\"Login_btn\"]:disabled'); 변경함
				$submit = opts.submitId ? $(opts.submitId) : $('button[id=\"Login_btn\"]:disabled');
		...
		// Passed in the form, the canvas and the canvas context
		// Scope (this) is active plugin options object (opts)
		onError: function($form, $canvas, ctx) { //실패 했을 경우.
			var opts = this;
			Login_Count = -3;
			return;
		}
