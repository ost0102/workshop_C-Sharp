...
if (typeof _view != 'undefined'){
	$(document).on("click","._btn_view", function(e){
		e.preventDefault();
		alert('view');
...
$(function(){	
	...
	if (typeof _edit != 'undefined'){
		$(document).on("click","._btn_edit", function(e){
			e.preventDefault();
	...
	/* 다중 선택 일 경우 */
	if (typeof _multi_del != 'undefined'){

		$("._multi_del").on('click',function(e){	
			e.preventDefault();
			var mode_txt = '정말로 삭제하시겠습니까?';
	...
	if (typeof _btn_init != 'undefined'){
		...
		$("._btn_init").on('click',function(e){	
			...
			if($(this).data('msg')){ 
				mode_txt = $(this).data('msg');
				var is_confirm = confirm( mode_txt);
			}else{ 
				var is_confirm = true;
			}
